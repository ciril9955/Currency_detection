...
def preprocess_image(image_path):
    try:
        ...
        st.error(f"Error: Image at {image_path} cannot be read or is not in a supported format.")
...
def classify_uploaded_image(latest_uploaded_image):
    if latest_uploaded_image is None:
        st.error("Error: No uploaded images found in the directory.")
        return
    ...
    st.write("Average Cosine Similarity with original dataset:", avg_similarity_original)
    st.write("Average Cosine Similarity with fake dataset:", avg_similarity_fake)

    if avg_similarity_original > ORIGINAL_SIMILARITY_THRESHOLD:
        st.success("The uploaded image is an original currency.")
    elif avg_similarity_fake < FAKE_SIMILARITY_THRESHOLD:
        st.error("The uploaded image is a fake currency.")
    else:
        st.warning("The authenticity of the uploaded image cannot be determined.")

# Streamlit App
def main():
    st.title("500-Currency Authenticity Checker")
    uploaded_image = st.file_uploader("Upload a currency image", type=["jpg", "png", "jpeg"])

    if uploaded_image is not None:
        with st.spinner("Checking the authenticity..."):
